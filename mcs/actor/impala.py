...
from mcs.actor.base.actor_module import ActorModule
...
class ImpalaHostActor(ActorModule, ACActorHelperMixin):
    ...
    def output_space(action_space):
        head_dict = {"critic": (1,), "pixel_control": (21 * 21, 6),**action_space}
    ...
    def compute_action_exp(self, preds, internals, obs, available_actions):
        ...
        log_softmaxes = []
        entropies = []
        pixel_control = preds["pixel_control"]
    ...
    def _exp_spec(cls, exp_len, batch_sz, obs_space, act_space, internal_space):
        ...
        spec = {
            ...
            "target_log_softmaxes": (exp_len, batch_sz, act_key_len, flat_act_space),
            "entropies": (exp_len, batch_sz, act_key_len),
            "values": (exp_len, batch_sz),
            "pixel_control": (exp_len, batch_sz, 21 * 21, 6),
            # From Workers
...
class ImpalaHostTargetActor(ActorModule, ACActorHelperMixin):
    args = {}
    ...
    def compute_action_exp(self, preds, internals, obs, available_actions):
        ...
        return (
            None,
            {
                "target_log_softmaxes": log_softmaxes
            },
        )
    ...
    def _exp_spec(cls, exp_len, batch_sz, obs_space, act_space, internal_space):
        ...
        spec = {
            "target_log_softmaxes": (exp_len, batch_sz, act_key_len, flat_act_space),
            "target_entropies": (exp_len, batch_sz, act_key_len),
            "target_values": (exp_len, batch_sz),
            "target_pixel_control": (exp_len, batch_sz, 21 * 21, 6),
            # From Workers
            "target_log_probs": (exp_len, batch_sz, act_key_len),
            **obs_spec,
            **action_spec,
            **internal_spec,
        }
