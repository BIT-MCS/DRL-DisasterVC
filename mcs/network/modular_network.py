...
from mcs.network.base.base import BaseNetwork
...
class ModularNetwork(BaseNetwork, metaclass=abc.ABCMeta):
    ...
    def forward(self, observation, internals):
        ...
        for key in self._output_keys:
            # if key == 'pixel_control':
            #     continue
            output = self.output_layers[key].forward(
                head_out_by_dim[len(self._output_space[key])]
            )
            output_by_key[key] = output
        # output_by_key['pixel_control'] = self.output_layers['pixel_control'].forward(head_out_by_dim[len(self._output_space[key])])
        #print(output_by_key['pixel_control'].shape)
        merged_internals = {}
